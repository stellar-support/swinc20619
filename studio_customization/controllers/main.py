...
class PaymentController(http.Controller):
    ...
    def stellar_form_feedback(self, **post):
        ...
        response = request.env['payment.transaction'].sudo().form_feedback(post, 'studio_customization')
        if response:
            return request.redirect('/payment/process')
        return request.redirect('/shop/payment')

    @http.route(['/payment/stellar/check_payment'], type='http', auth="user", website=True)
